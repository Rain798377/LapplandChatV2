...
from config import AUTOPLAY_DELAY, SPOTIFY_CLIENT_ID, SPOTIFY_CLIENT_SECRET, MAX_FILE_SIZE_MB
...
def _build_search_attempts(query: str) -> list[str]:
    ...
    # Strip known noise tags that confuse YouTube search
    NOISE_RE = re.compile(
        r"[\[\(]"
    ...
    # 1. Cleaned query — best signal for YouTube
    attempts.append(f"ytsearch5:{cleaned}")

    # 2. Raw original, if different
    if raw != cleaned:
